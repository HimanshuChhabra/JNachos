...
public class AddrSpace {
	...
	public AddrSpace(OpenFile executable) {
		...
		for (int i = 0; i < mNumPages; i++) {
			...
			mPageTable[i].swapLoc = JNachos.getNextSwapCount();
			...
			if(i == 0){
				...
				if(mPageTable[i].physicalPage != -1){
					...
					//}
						...
						JNachos.getPhyPageList().add(mPageTable[i].physicalPage);
						
						//Updating Map
						JNachos.setPhyPageNumToPorcessID(mPageTable[i].physicalPage, JNachos.getCurrentProcess().getPid());
				}else{
	...
	public AddrSpace(AddrSpace pToCopy) {
		...
		// pages to be read-only
			...
			if (i == 0) {
				...
				if (mPageTable[i].physicalPage != -1) {
					//Add the Page to the used phy list
					JNachos.getPhyPageList().add(mPageTable[i].physicalPage);
			...
			//Read from Parents Swap Space location in to the buffer
			JNachos.getSwapFile().readAt(buffer, Machine.PageSize, pToCopy.mPageTable[i].swapLoc * Machine.PageSize);
			
			//Write into the Childs Swap Space location from the buffer
			JNachos.getSwapFile().writeAt(buffer, Machine.PageSize, mPageTable[i].swapLoc * Machine.PageSize);
			}
	}
	...
	public TranslationEntry getPageTableEntry(int virtualPageNumber){
		TranslationEntry entry = null ;
		try{
		 entry =  mPageTable[virtualPageNumber];
		}
		catch(ArrayIndexOutOfBoundsException e){
			
			Debug.print('t', "Virtual Page Number Entry Not Found in the Page Table");
			System.out.println("Virtual Page Number Entry Not Found in the Page Table");
		}
		return entry;
	}
	
	public TranslationEntry[] getPageTableByProcess(){
		return mPageTable;
	}
}