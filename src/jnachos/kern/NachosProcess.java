...
public class NachosProcess implements Runnable {
	...
	private static LinkedList<NachosProcess> allProcesses = new LinkedList<NachosProcess>();
	
	public static final String PrevPCReg = "PrevPCReg";
	public static final String PCReg = "PCReg";
	public static final String NextPCReg = "NextPCReg";
	...
	// what should be its execution routine, I have used same as parent?, we need to change the interrupt levels right?
	public synchronized int fork(){
		NachosProcess currentProcess = JNachos.getCurrentProcess();
		Debug.print('t', "Creating a child process for process:" + currentProcess.getName());
		NachosProcess childProcess = new NachosProcess("Child of process:"+currentProcess.getPid());
		// Mapping of address spaces and User registers
		AddrSpace childspace = new AddrSpace(currentProcess.getSpace());
		childProcess.setSpace(childspace);
		
		// increment PC for the parent after that copy all the user registers into the child
		updateCounters();
				
		//Copy the State of parent to the child
		childProcess.saveUserState();
		//Send 0 as the return value to the user program
		childProcess.mUserRegisters[2] = 0;

		//create the child thread
		childProcess.fork(new StartChildProcess(), currentProcess.getName());
	
		// need to set the correct PCs, need to set the read 2 for both child and parent
		return childProcess.getPid();
	}

	
	/**
	 * Update the program counters in case of successful completion of the fork sys call.
	 */
	public static void updateCounters(){
		int pcNext = Machine.mRegisters[Machine.NextPCReg] + 4;
		Machine.mRegisters[Machine.PrevPCReg] = Machine.mRegisters[Machine.PCReg]; // for debugging
		Machine.mRegisters[Machine.PCReg] = Machine.mRegisters[Machine.NextPCReg];
		Machine.mRegisters[Machine.NextPCReg] = pcNext;
	}
	 ...
	 * @return
	 */
	public synchronized int join(int pid){
		 
		NachosProcess currentProcess = JNachos.getCurrentProcess();
		 
		if(pid != currentProcess.getPid() && pid !=0 &&NachosProcess.hasProcess(pid)){ 
			
			System.out.println("Process with PID: "+currentProcess.getPid()+" is sleeping for PID: "+ pid + " to finish");
			currentProcess.sleep() ;
			// need to return something.
		}
		
		return currentProcess.getUserRegisters()[2];
	}
	
	/**
	 * To check if the process exists
	 */
	public static boolean hasProcess(int pid){
		NachosProcess process = getProcessByID(pid);
	...
	public void finish() {
		...
		//Free used Pages in bitmap
	    /**
	     * Allocated pages must be freed and main memory data must be zeroed
	     * regardless of process being successfully terminated or not, to ensure
	     * there is no information leak.
	     */
		SystemCallHandler.freePhysicalPages();
		}
	 ...
	 * 
	 * @return long process Id for this thread.
	 */
	 ...
	 * 
	 * @return int process Id for this thread.
	 */
	public int getPid() {
		return mpid;
	}
